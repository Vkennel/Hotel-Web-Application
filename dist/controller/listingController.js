...
async function List(req, res, next) {
    ...
    try {
        ...
        const record = await list_1.ListInstance.create({ id, ...req.body,
            userId: verified.id,
        });
        res.status(201).json({
...
async function getList(req, res, next) {
    try {
        ...
        const record = await list_1.ListInstance.findAndCountAll({ limit, offset, include: [{ model: list_1.ListInstance,
                    attributes: ['id', "fullName", "email", "phoneNumber"], as: "user" }] }); //list is the name of my list file in database
        //   where: {},
        //   limit,
        //   offset,
        // });
